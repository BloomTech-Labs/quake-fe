    ...
    if (quakeFetchError === false) {
        ...
        if (quakeFetch === false) {
            ...
            if (quakes.length === 0) {
                ...
                return (
                    <div>Looks Like We Couldn't Find Any Quakes...</div>
                );
            } else {
                ...
                return (
                    quakes.map((quake, index) => {
                        return (
                            <Card quake={quake} number={index} key={index} />
                        );
                    })
                );
            }
            ...
            return (
                <div>Fetching Quakes...</div>
            );
        }
    } else {
        console.log("Fetch Error", quakeFetchError);
        return (
            <div>There was a problem getting your quakes...</div>
        );
    }
}
