...
function Card({ quake, number }) {
  ...
  return (
    <main>
      <article tabIndex="0" aria-label="earthquake information" className="activity-card" onClick={() => setOpen((open) => !open)}>
        <aside tabIndex="0" aria-label={`earthquake magnitude = ${Math.round(quake.properties.mag * 10) / 10}`} className="magnitude">
          {Math.round(quake.properties.mag * 10) / 10}
        </aside>
          <article tabIndex="0" aria-label="aria-info" className="place-info">
            <h2 id="aria-info" tabIndex="0" className="city">{split2[0]}</h2>
            <h2 id="aria-info" tabIndex="0" className="country">{split2[1]}</h2>
            <h3 id="aria-info" tabIndex="0" className="distance">{split2[2]}</h3>
          </article>
        <div
